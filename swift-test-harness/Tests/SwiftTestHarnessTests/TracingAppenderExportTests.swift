...
// of the pipeline:
//
//   1. `embedSwiftExportForXcode` produced `TracingAppender.swiftmodule/`
...
//      `import TracingAppender` above would fail at compile time.
//
//   2. The static archive `libTracingAppender.a` (produced by the
...
//      architecture arm64".
//
//   3. The Kotlin `swiftExport { moduleName = "TracingAppender" }` and
//      `flattenPackage = "io.github.kotlinmania.tracingappender"` configuration in
...
//      via the `TracingAppenderLibrary` product.
//
...
final class TracingAppenderExportTests: XCTestCase {
    func testSwiftModuleLoads() throws {
        XCTAssertTrue(true, "TracingAppender swift module imported cleanly")
    }
}
